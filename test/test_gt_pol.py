...
def GetPolsToCompare(sim_name, 
    ...
    hpc = mwf1.get_td_waveform(total_mass=total_mass, distance=distance, inclination=inclination,
    ...
    print('Loading waveform through LAL')
    hp_l, hx_l = get_td_waveform(approximant='NR_hdf5',
                             numrel_data=file,
                             mass1=params['mass1'],
                             mass2=params['mass2'],
                             spin1x=S1[0],
    ...
    wfm3.load_modes(ftype='GT', var_type='Strain', resam_type=resam_type, interp_kind='cubic')

    time_w3, hp_w3, hx_w3 = wfm3.to_td_waveform(Mtotal=total_mass, 
                                            distance=distance, 
                                            theta=angles['theta'], 
                                            phi=angles['psi'], 
                                            alpha = angles['alpha'], 
    ...
    # print('Max locations before centering')
    # print(imax_n, imax_l, imax_w3)
    ...
    # print('Max locations after centering')
    # print(imax_n, imax_l, imax_w3)
    ...
    return {"nrcat": [t_n, wf_n, a_n, p_n], "wftools" : [t_w3, wf_w3, a_w3, p_w3], "lal": [t_l, wf_l, a_l, p_l]}
...
class TestGTPol(unittest.TestCase):
    ...
    def test_pol(self):
        ...
        inc_angles = [0.0015, 0.5, 1, 1.5, np.pi/2 - 0.001, np.pi/2, np.pi/2+0.0001, 2.5, 3, np.pi-0.0015] 
        ...
        for inclination in inc_angles:
            for coa_phase in coa_phases:
                ...
                message("\n--------------------------")
                message(f"inclination {inclination} Coa phase {coa_phase}")
                message("--------------------------")

                waveforms = GetPolsToCompare(sim_name=sim_name,
                ...
                message(f"Mismatches are nrcatalogtools - lal {mm_nl}%", message_verbosity=1) 
                if test_wrt_wftools:
