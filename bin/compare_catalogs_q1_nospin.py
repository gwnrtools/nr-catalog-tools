...
def load_waveform(cat, sim_name, label, is_sxs=False):
    ...
    try:
        ...
        print(f"    Loaded. {len(lm_list)} modes available: "
...
for ell, em in MODES:
    ...
    if len(raw_peaks) >= 2:
        ...
        for i in range(len(keys)):
            for j in range(i + 1, len(keys)):
                a, b = keys[i], keys[j]
                ratio = raw_peaks[a] / raw_peaks[b] if raw_peaks[b] > 0 else float("nan")
    ...
    for label, (t, amp, phase, idx0) in phase_infos.items():
        ...
        print(f"    [{label}]  phi[-0.1 s..0] = {drift:.4f} rad  "
    ...
    if len(drift_vals) >= 2:
        ...
        for i in range(len(keys)):
            for j in range(i + 1, len(keys)):
                ...
                pct  = abs(diff / drift_vals[b]) * 100 if drift_vals[b] != 0 else float("nan")
                flag = ""
                if abs(diff) > 0.5:
                    flag = "  ← >0.5 rad DISCREPANCY"
                elif abs(diff) > 0.1:
                    flag = "  ← >0.1 rad discrepancy"
                print(f"    phase drift diff {a}-{b} = {diff:+.4f} rad  "
                      f"({pct:.2f}%){flag}")
...
for ell, em in MODES:
    ...
    for label, (t, amp, phase, idx0) in phase_infos.items():
        ...
        ax_norm.plot(t, amp / amp.max() if amp.max() > 0 else amp,
                     color=c, ls=ls, lw=1.0, label=label)
    ...
    ax_amp.set_title(
        ...
        rf"[M={TOTAL_MASS} M$\odot$, d={DISTANCE} Mpc]", fontsize=12)
    ax_norm.set_ylabel(rf"$|h_{{{ell}{em}}}|$ / peak", fontsize=11)
    ax_norm.set_title("Normalized amplitude shape", fontsize=12)
    ax_phase.set_ylabel(rf"$\phi_{{{ell}{em}}} - \phi_{{{ell}{em}}}(t=0)$ [rad]", fontsize=11)
    ...
    subtitle = "  |  ".join(f"{lab}: {sim_labels[lab]}" for lab in ["RIT", "SXS", "MAYA"])
    fig.suptitle(
...
for k, (a, b) in enumerate(pairs):
    ...
    offset = (k - 1) * width
    ax.bar(x + offset, ratios, width, label=f"{a}/{b}",
           color=pair_colors[k], alpha=0.8)
...
ax.set_title(f"Cross-catalog amplitude ratios per mode\n"
...
print("=" * 70)
header = f"{'Mode':8}  {'RIT raw':10}  {'SXS raw':10}  {'MAYA raw':10}  " \
...
print("=" * 70)
header2 = f"{'Mode':8}  {'RIT [rad]':12}  {'SXS [rad]':12}  {'MAYA [rad]':12}  " \
